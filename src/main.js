...
ipcMain.handle("updateExchange", async (event, { id, newValue }) => {
  const currencyUpdated = await currencyServices.updateExchange({
    id,
    newValue,
  });
...
ipcMain.handle(
  "paginate",
  async (event, config) => await services.paginate(config)
);
...
ipcMain.handle(
  "deleteProduct",
  async (event, id) => await services.deleteProduct(id)
);

ipcMain.handle("newProductHaveCreated", () => window.reload());
